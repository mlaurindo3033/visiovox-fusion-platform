...
# Worker específico para conversão de modelos ONNX para TensorRT
class TensorRTConversionWorker(qtc.QThread):
    # Sinais para comunicação com a thread principal
    progress_signal = qtc.Signal(int, str)  # Progresso (0-100) e mensagem de status
    status_signal = qtc.Signal(str)
    completion_signal = qtc.Signal(bool, str)  # Sucesso/Falha, Caminho do arquivo resultante
    
    def __init__(self, onnx_path, trt_path, precision="fp16", custom_plugin_path=None, verbose=False):
        super().__init__()
        self.onnx_path = onnx_path
        self.trt_path = trt_path
        self.precision = precision
        self.custom_plugin_path = custom_plugin_path
        self.verbose = verbose
        
    def run(self):
        """Executa a conversão em uma thread separada"""
        try:
            self.status_signal.emit(f"Convertendo modelo para TensorRT\nEsta operação pode demorar vários minutos na primeira execução.")
            self.progress_signal.emit(2, "Inicializando bibliotecas TensorRT")
            
            # Importa as dependências dentro da thread para evitar problemas de concorrência
            from app.processors.utils.engine_builder import onnx_to_trt
            
            # Define uma callback para atualizar o progresso e status
            def update_progress(progress, message=None):
                status_text = message if message else f"Convertendo modelo: {progress}% concluído"
                self.progress_signal.emit(progress, status_text)
                
                # Importante: permite que a UI processe eventos durante a conversão
                # mesmo dentro da thread de trabalho
                qtc.QCoreApplication.processEvents()
                
            # Executa a conversão propriamente dita, na thread secundária
            onnx_to_trt(
                onnx_model_path=self.onnx_path,
                trt_model_path=self.trt_path,
                precision=self.precision,
                custom_plugin_path=self.custom_plugin_path,
                verbose=self.verbose,
                progress_callback=update_progress
            )
            
            # Sinaliza conclusão bem-sucedida
            self.completion_signal.emit(True, self.trt_path)
            
        except Exception as e:
            import traceback
            traceback.print_exc()
            self.status_signal.emit(f"Erro na conversão: {str(e)}")
