...
class ParameterResetDefaultButton(QtWidgets.QLabel):
    def __init__(self, related_widget: ParameterSlider | ParameterDecimalSlider | SelectionBox, *args, **kwargs):
        ...
        self.setCursor(QtCore.Qt.PointingHandCursor)
        self.setToolTip('Resetar')
        gif_path = 'C:/Users/miche/Music/VisoMaster/icons/reset.gif'
        print(f"[DEBUG] Tentando carregar GIF: {gif_path}")
        self.movie = QMovie(gif_path)
        if not self.movie.isValid():
            print(f"[ERRO] GIF inválido: {gif_path}")
            self.setText('X')
            self.setStyleSheet('background: #f00; color: #fff;')
        else:
    ...
    def mousePressEvent(self, event):
        if event.button() == QtCore.Qt.LeftButton:
            self.related_widget.reset_to_default_value()
        super().mousePressEvent(event)

    def setText(self, text):
        print(f"[ALERTA] setText chamado em ParameterResetDefaultButton: {text}")
        super().setText(text)
