...
from app.utils.sam2_face_segmenter import SAM2FaceSegmenter
if TYPE_CHECKING:
...
class FaceMasks:
    def __init__(self, models_processor: 'ModelsProcessor'):
        ...
        self.sam2_foi_usado = False  # Flag para indicar se o SAM2 foi utilizado
    ...
    def apply_face_parser(self, img, parameters):
        method = parameters.get('FaceSegmentationMethodSelection', 'Face Parsing Tradicional')
        if method == 'Nenhum':
            print("LOG: Segmentação de rosto desativada (Nenhum). Usando máscara branca.")
            return torch.ones((1, 512, 512), dtype=torch.float32)
        if method == 'SAM 2':
            model_name = parameters.get('SAM2ModelSelectionSelection', 'Rápido (Tiny)')
            if not hasattr(self, '_sam2_segmenter') or getattr(self, '_sam2_model_name', None) != model_name:
                self._sam2_segmenter = SAM2FaceSegmenter(model_name)
                self._sam2_model_name = model_name
            # img pode ser torch.Tensor ou np.ndarray
            if isinstance(img, torch.Tensor):
                img_np = img.permute(1, 2, 0).cpu().numpy().astype(np.uint8)
            else:
                img_np = img
            mask, n_masks, max_area = self._sam2_segmenter.segment_face(img_np, return_info=True)
            self.sam2_foi_usado = True  # Marca que o SAM2 foi utilizado
            print(f"[DEBUG] ===> SAM 2 ATIVO! Segmentação de rosto realizada com SAM 2 (modelo: {model_name}). Máscaras encontradas: {n_masks}, maior área: {max_area} pixels.")
            print(f"LOG: Usando SAM 2 (modelo: {model_name}) para segmentação de rosto. Encontradas {n_masks} máscaras, maior área: {max_area} pixels.")
            mask = torch.from_numpy(mask).float()
            if mask.max() > 1:
                mask = mask / 255.0
            mask = mask.unsqueeze(0)
            mask = torch.nn.functional.interpolate(mask.unsqueeze(0), size=(512, 512), mode='bilinear', align_corners=False).squeeze(0)
            return mask
        else:
            print("LOG: Usando Face Parsing Tradicional para segmentação de rosto.")
    ...
    def foi_usado_sam2(self):
        """Retorna True se o SAM2 foi utilizado desde a última verificação e reseta a flag."""
        usado = self.sam2_foi_usado
        self.sam2_foi_usado = False
        return usado
