...
class FrameWorker(threading.Thread):
    ...
    def get_face_similarity_tform(self, swapper_model: str, kps_5: np.ndarray) -> trans.SimilarityTransform:
        print(f"get_face_similarity_tform - swapper_model: {swapper_model}, kps_5 shape: {kps_5.shape}, kps_5: {kps_5}")
        ...
        if swapper_model != 'GhostFace-v1' and swapper_model != 'GhostFace-v2' and swapper_model != 'GhostFace-v3' and swapper_model != 'CSCS':
            ...
            # Template específico para CSCS usando FFHQ_kps como no código original
            tform.estimate(kps_5, self.models_processor.FFHQ_kps)
        else:
            # Lógica específica para GhostFace
            ...
            tform.params[0:2] = M
        print(f"get_face_similarity_tform - M calculated: {tform.params}")
        return tform
    ...
    def get_affined_face_dim_and_swapping_latents(self, original_faces: tuple, swapper_model, dfm_model, s_e, t_e, parameters,):
        ...
        if swapper_model == 'Inswapper128':
            ...
            # Restaurar a lógica original de seleção de resolução
            dim = 1  # Valor padrão
            if 'SwapperResSelection' in parameters:
                ...
                else:  # Fallback para 128 se a seleção for inválida
                    dim = 1
                    input_face_affined = original_face_128
            else:  # Fallback para 128 se o parâmetro não existir
                dim = 1
                input_face_affined = original_face_128
                
            # Log para diagnóstico
            print(f"Inswapper128 - dim: {dim}, input_face_affined shape: {input_face_affined.shape}")
            ...
            # O modelo CSCS espera entrada de 256x256
            dim = 2
            input_face_affined = original_face_256
            print("---> [CSCS] Usando dim=2 e input_face_affined para 256x256")
    ...
    def get_swapped_and_prev_face(self, output, input_face_affined, original_face_512, latent, itex, dim, swapper_model, dfm_model, parameters, ):
        print(f"get_swapped_and_prev_face - swapper_model: {swapper_model}, input_face_affined shape: {input_face_affined.shape}, dim: {dim}")
        # original_face_512, original_face_384, original_face_256, original_face_128 = original_faces
        ...
        if swapper_model == 'Inswapper128':
            ...
            print(f"Inswapper128 - input_face_disc shape: {input_face_disc.shape}, min: {input_face_disc.min()}, max: {input_face_disc.max()}")
            print(f"Inswapper128 - swapper_output shape: {swapper_output.shape}, min: {swapper_output.min()}, max: {swapper_output.max()}")
            ...
            for k in range(itex):
                # Simplificar o processamento para seguir a lógica do código original
                input_face_disc = input_face_affined.permute(2, 0, 1)
                # Normalizar para [-1, 1] diretamente
                ...
                print(f"CSCS - input_face_disc (entrada para inferência): shape={input_face_disc.shape}, min={input_face_disc.min():.4f}, max={input_face_disc.max():.4f}, mean={input_face_disc.mean():.4f}")
                
                # Preparar o tensor de saída
                swapper_output = torch.empty((1,3,256,256), dtype=torch.float32, device=self.models_processor.device).contiguous()
                
                # Converter latent para numpy array se necessário
                ...
                self.models_processor.run_swapper_cscs(input_face_disc, latent_numpy, swapper_output)
                
                # Log para diagnóstico da SAÍDA BRUTA do modelo
                print(f"CSCS - swapper_output (saída bruta da inferência): shape={swapper_output.shape}, min={swapper_output.min():.4f}, max={swapper_output.max():.4f}, mean={swapper_output.mean():.4f}")
                
                # Processar a saída - remover a dimensão do batch
                swapper_output = torch.squeeze(swapper_output) # Agora é [3, 256, 256]
                
                # Logs para canais individuais (ANTES da desnormalização)
                print(f"CSCS - Canal R (bruto, squeeze): min={swapper_output[0].min().item():.4f}, max={swapper_output[0].max().item():.4f}, mean={swapper_output[0].mean().item():.4f}")
                print(f"CSCS - Canal G (bruto, squeeze): min={swapper_output[1].min().item():.4f}, max={swapper_output[1].max().item():.4f}, mean={swapper_output[1].mean().item():.4f}")
                print(f"CSCS - Canal B (bruto, squeeze): min={swapper_output[2].min().item():.4f}, max={swapper_output[2].max().item():.4f}, mean={swapper_output[2].mean().item():.4f}")

                # Verificar o intervalo de valores da saída para determinar a desnormalização correta
                min_val = swapper_output.min().item()
                max_val = swapper_output.max().item()
                print(f"CSCS - valores globais (após squeeze): min={min_val:.4f}, max={max_val:.4f}")
                
                # Desnormalizar a saída dependendo do intervalo de valores
                if min_val >= -1.5 and max_val <= 1.5: # Intervalo [-1, 1] esperado
                    print("CSCS - Aplicando desnormalização de [-1, 1] para [0, 1]")
                    swapper_output_desnorm = torch.add(torch.mul(swapper_output, 0.5), 0.5)
                elif min_val >= 0.0 and max_val <= 1.0:
                    print("CSCS - Saída já parece estar no intervalo [0, 1], não desnormalizando")
                    swapper_output_desnorm = swapper_output
                else:
                    print(f"CSCS - Intervalo inesperado: [{min_val:.4f}, {max_val:.4f}]. Clampeando para [0,1] após tentativa de desnormalização padrão.")
                    # Tenta a desnormalização padrão e depois clampa, como segurança
                    swapper_output_desnorm = torch.add(torch.mul(swapper_output, 0.5), 0.5)
                    swapper_output_desnorm = torch.clamp(swapper_output_desnorm, 0.0, 1.0)
                
                print(f"CSCS - swapper_output (após desnormalização para [0,1]): min={swapper_output_desnorm.min():.4f}, max={swapper_output_desnorm.max():.4f}, mean={swapper_output_desnorm.mean():.4f}")

                # Reordenar os canais para HWC
                swapper_output_desnorm_hwc = swapper_output_desnorm.permute(1, 2, 0)
                
                # Atualizar as variáveis para a próxima iteração
                prev_face = input_face_affined.clone() # input_face_affined é [H, W, C] e [0,1]
                input_face_affined = swapper_output_desnorm_hwc.clone()

                # Preparar a saída final
                output = swapper_output_desnorm_hwc.clone()
                output = torch.mul(output, 255)
                output = torch.clamp(output, 0, 255)
                
                # Log final
                print(f"CSCS - output final (0-255 HWC): min={output.min():.4f}, max={output.max():.4f}, mean={output.mean():.4f}")

        elif swapper_model == 'DeepFaceLive (DFM)' and dfm_model:
            try:
                # Garantir que dfm_model seja um objeto de modelo, não uma string
                if isinstance(dfm_model, str):
                    print(f"Carregando modelo DFM: {dfm_model}")
                    dfm_model_obj = self.models_processor.load_dfm_model(dfm_model)
                    if dfm_model_obj is None:
                        raise ValueError(f"Falha ao carregar modelo DFM: {dfm_model}")
                    out_celeb, _, _ = dfm_model_obj.convert(original_face_512, parameters['DFMAmpMorphSlider']/100, rct=parameters['DFMRCTColorToggle'])
                else:
                    # Usar o objeto de modelo diretamente
                ...
                print(f"Erro ao processar modelo DFM: {str(e)}")
                import traceback
                print(traceback.format_exc())
                # Fallback: usar a face original
                prev_face = input_face_affined.clone()
                output = input_face_affined.clone()
                output = torch.mul(output, 255)
                output = torch.clamp(output, 0, 255)
    ...
    def swap_core(self, img, kps_5, kps=False, s_e=None, t_e=None, parameters=None, control=None, dfm_model=False): # img = RGB
        # Verificar e corrigir o valor do sigma antes de usar
        # Extrair parâmetros de borramento
        blur_amount = parameters['BlurAmountSlider'] if parameters and 'BlurAmountSlider' in parameters else 5
        sigma = blur_amount
        
        # Garantir que sigma seja positivo
        if sigma <= 0:
            sigma = 1.0  # Usar um valor padrão seguro se o valor for inválido
        
        # Usar valor seguro para kernel_size também
        kernel_size = max(3, int(blur_amount * 2) + 1)
        kernel_size = kernel_size if kernel_size % 2 == 1 else kernel_size + 1  # Garantir que seja ímpar
        ...
        if parameters['FinalBlendAdjEnableToggle'] and parameters['FinalBlendAdjEnableToggle'] > 0:
            ...
            sigma = max(0.1, final_blur_strength * 0.1)  # Sigma proportional zur Stärke, mas nunca zero ou negativo
            # Gaussian Blur anwenden
            try:
                gaussian_blur = transforms.GaussianBlur(kernel_size=kernel_size, sigma=sigma)
                swap = gaussian_blur(swap)
            except ValueError as e:
                print(f"Erro ao aplicar GaussianBlur: {e}")
                print(f"Parâmetros problemáticos: kernel_size={kernel_size}, sigma={sigma}")
                # Usar valores seguros se falhar
                kernel_size = 3
                sigma = 1.0
                gaussian_blur = transforms.GaussianBlur(kernel_size=kernel_size, sigma=sigma)
                swap = gaussian_blur(swap)
